...
class AdminManager:
        ...
        try:
            with self.db_manager.connect() as conn:
                with conn.cursor() as cur:
                    ...
                    print(f"Doktor eklendi. ID: {next_id}, Bölüm ID: {bolum_id}")
    ...
    def add_department(self, ad):
        try:
            with self.db_manager.connect() as conn:
                with conn.cursor() as cur:
                    cur.execute("SELECT COALESCE(MAX(bolum_id), 0) + 1 FROM emre_odev.bolum")
                    next_id = cur.fetchone()[0]

                    cur.execute("""
                        INSERT INTO emre_odev.bolum 
                        (bolum_id, ad)
                        VALUES (%s, %s)
                        RETURNING bolum_id
                    """, (next_id, ad))
    ...
    def delete_department(self, dept_id):
        try:
            with self.db_manager.connect() as conn:
                with conn.cursor() as cur:
                    cur.execute("SELECT COUNT(*) FROM emre_odev.doktor WHERE bolum_id = %s", (dept_id,))
                    doctor_count = cur.fetchone()[0]

                    if doctor_count > 0:
                        raise Exception("Bu bölümde kayıtlı doktorlar var. Önce doktorları silmeniz gerekiyor.")

                    cur.execute("DELETE FROM emre_odev.bolum WHERE bolum_id = %s", (dept_id,))
                    conn.commit()
        except psycopg2.Error as e:
            print("Veritabanı hatası:", e.pgerror)
            raise Exception(f"Veritabanı hatası: {str(e)}")
...
class DoctorManager:
    ...
    def get_doktor_by_id(self, doktor_id):
        with self.db_manager.connect() as conn:
            with conn.cursor() as cur:
...
class AppointmentManager:
    ...
    def get_working_hours(self, doktor_id, tarih):
        with self.db_manager.connect() as conn:
            with conn.cursor() as cur:
                ...
                try:
                    calisma_saatleri = json.loads(doktor_calisma_saatleri[0])
                ...
                baslangic, bitis = calisma_saatleri[gun_adi].split("-")
    ...
    def get_available_hours(self, doktor_id, tarih):
        working_hours = self.get_working_hours(doktor_id, tarih)
...
def get_doctor_details(doctor_id):
    if 'admin_id' not in session:
        return jsonify({'error': 'Yetkisiz erişim'}), 401
    try:
        doctor = admin_manager.get_doctor_by_id(doctor_id)
        if doctor:
...
def update_doctor(doctor_id):
    ...
    try:
        ...
        return jsonify({'success': True, 'message': 'Doktor başarıyla güncellendi'})
    except Exception as e:
        return jsonify({'error': str(e)}), 500
...
def add_department():
    ...
    try:
        data = request.get_json()
        if not data.get('ad'):
            return jsonify({'error': 'Bölüm adı gereklidir'}), 400

        department_id = admin_manager.add_department(data['ad'])
        return jsonify({'success': True, 'message': 'Bölüm başarıyla eklendi'})
    except Exception as e:
        return jsonify({'error': str(e)}), 500
...
def delete_department(dept_id):
    if 'admin_id' not in session:
        return jsonify({'error': 'Yetkisiz erişim'}), 401
    try:
        admin_manager.delete_department(dept_id)
        return jsonify({'success': True, 'message': 'Bölüm başarıyla silindi'})
    except Exception as e:
        return jsonify({'error': str(e)}), 500
@app.route('/admin/doctors/add', methods=['POST'])
...
def book_appointment():
    ...
    try:
        ...
        return f"Bir hata oluştu: {str(e)}"
@app.route('/_get_doctors')
def get_appointments_by_doctor():
    bolum_id = request.args.get('bolum_id')
    try:
        doctors = appointment_manager.get_doctors_by_department(bolum_id)
        return jsonify([{"doktor_id": doc[0], "ad": doc[1], "soyad": doc[2]} for doc in doctors])
    except Exception as e:
